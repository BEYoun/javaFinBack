...
public class EtapeManagement {
	...
	public ObservableList<EtapeTable> getDataEtapeFromSqlAndAddToObservableList() {
		...
		for (Etape etape : monArray) {
			if (etape.getEmp() != null) {
				...
				adminTableData.add(new EtapeTable(etape.getId(), etape.getNom(), etape.getProcess().getNom(),
						etape.getOrdre(), etape.getEmp().getFirstName(), etape.getEmp().getLastName(),
						etape.getEmp().getEmail()));
			} else {
				adminTableData.add(new EtapeTable(etape.getId(), etape.getNom(), etape.getProcess().getNom(),
						etape.getOrdre(), "non Affecter", "non Affecter", "non Affecter"));
			}
	...
	public ObservableList<String> getDataProcessusFromSqlToObsevableList() {
		...
		availableChoices = etdao.getAllNom();
		return availableChoices;
	}

//	public Etape find(int id_etape) {
//		// TODO Auto-generated method stub
//		//return etdao.find(id_etape);
//	}
